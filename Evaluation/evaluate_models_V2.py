...
RESULT_FILE = HOME / "code/Master_Thesis/evaluation_results_V2.txt"
    ...
    for set_name in TEST_SETS:
        ...
        for i in range(0, len(MODELS), 5):
            ...
            for model_name in model_group:
                ...
                # 1. KORREKTE NORMALISIERUNG (Pro Slice via axis=(1,2))
                ...
                # 2. CLIPPING auf [0, 1] für Konsistenz mit Training & Metriken
                preds = np.clip(preds, 0.0, 1.0)
                y_norm = np.clip(y_norm, 0.0, 1.0)

                # 3. PER-SAMPLE METRIKEN BERECHNEN (Wichtig für PSNR & SSIM)
                y_t = tf.convert_to_tensor(y_norm)
                y_p = tf.convert_to_tensor(preds)
                
                # MSE pro Bild berechnen
                mse_per_sample = np.mean(np.square(preds - y_norm), axis=(1, 2, 3))
                
                # PSNR pro Bild berechnen (analog zum Training)
                psnr_per_sample = 10.0 * np.log10(1.0 / (mse_per_sample + 1e-12))
                
                # SSIM pro Bild berechnen (tf.image.ssim gibt einen Vektor zurück)
                ssim_per_sample = tf.image.ssim(y_t, y_p, max_val=1.0).numpy()

                # Durchschnittswerte für die Tabelle
                loss_v = float(mae_ssim_2d(y_t, y_p).numpy())
                mae_v  = float(np.mean(np.abs(preds - y_norm)))
                mse_v  = float(np.mean(mse_per_sample))
                ssim_v = float(np.mean(ssim_per_sample))
                psnr_v = float(np.mean(psnr_per_sample))
