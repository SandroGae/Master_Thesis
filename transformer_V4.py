...
# -----------------------------
# Modellaufbau (Heavyweight U-Transformer)
# -----------------------------
def build_srdtrans_swin(input_shape=(192, 240, 5), embed_dim=96):
    ...
    # Ebene 3 (Bottleneck: 48x60) -> window_size=4 wegen Teilbarkeit
    ...
    return models.Model(inputs, outputs, name="srdtrans_V4_Heavyweight")

# -----------------------------
# Daten-Pipeline & Loss (UNet analog)
# -----------------------------
...
RUN_NAME = f"transformer_V4__BS{BATCH_SIZE}__seed{SEED}__emb{EMBED_DIM}__lossMAE_SSIM__{RUN_ID}"
...
meta = make_meta_dict(
    ...
    extra={"loss": "mae_ssim(alpha=0.6)", "model": "SRDTrans_V4_Heavyweight_UformerStyle"}
)
