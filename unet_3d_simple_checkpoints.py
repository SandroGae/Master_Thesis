...
def finalize_run(
    ...
    # PSNR-Feld robust finden (z.B. val_psnr, val_psnr_metric, val_psnr_metric_3d, etc.)
    best_val_psnr = None
    psnr_keys = [k for k in hist.keys() if k.startswith("val_") and "psnr" in k.lower()]
    if psnr_keys:
        # nimm den ersten passenden Key deterministisch
        psnr_key = sorted(psnr_keys)[0]
        best_val_psnr = float(hist[psnr_key][best_idx])
