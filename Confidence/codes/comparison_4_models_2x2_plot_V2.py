...
def load_model_data(name, folder, prefix):
    ...
    for s_id, files in tqdm(series_dict.items(), desc=f"Verarbeite {name}"):
        ...
        global_raw_err.append((gt - mu_ens).flatten()) # Für Z-Score Berechnung (mit Vorzeichen)
...
def create_split_dashboards(data_dict):
    ...
    # --- Plot 3: Sparsification (Epistemic) ---
    ax = axes_mu[2]
    ...
    for name in names:
        ...
        for f in fractions:
            ...
            rem_sem.append(np.std(r_err) / np.sqrt(len(r_err))) # SEM
    ...
    save_path_mu = OUT_DIR / "Mu_Channel_Evaluation.png"
    ...
    # --- Plot 1: Reliability Diagram (Calibration) ---
    ...
    for name in names:
        ...
        for i in range(len(bins)-1):
            ...
            if n_pix > 1000:
                ...
                obs_sem.append(np.std(b_err) / np.sqrt(n_pix)) # SEM
    ...
    # --- Plot 2: Z-Score Histogram ---
    ax = axes_sig[1]
    x_ideal = np.linspace(-5, 5, 200)
    # Einzeichnen der idealen N(0,1) Normalverteilung
    ...
    for name in names:
        ...
        # Berechnung des Z-Scores: (GT - Pred) / Sigma
    ...
    # --- Plot 3: Expected Calibration Error (ECE) Bar Chart ---
    ax = axes_sig[2]
    ece_values = []
    ...
    for name in names:
        ...
        ece_score = 0
        total_valid = 0
        # ECE berechnet die gewichtete Differenz zwischen RMSE und vorhergesagtem Sigma
        for i in range(len(bins)-1):
            mask = (alea >= bins[i]) & (alea < bins[i+1])
            n_pix = np.sum(mask)
            if n_pix > 1000:
    ...
    plt.tight_layout(pad=3.0)
    save_path_sig = OUT_DIR / "Sigma_Channel_Evaluation.png"
