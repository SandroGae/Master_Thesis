...
for alpha_val in ALPHA_LIST:
    for beta_val in BETA_LIST:
        ...
        # Werte runden für den Vergleich und die Pfadnamen
        a_r = round(float(alpha_val), 3)
        b_r = round(float(beta_val), 3)

        # Prüfen, ob der Punkt bereits existiert (Bestand)
        if (a_r, b_r) in SKIP_POINTS:
            print(f">>> ÜBERSPRINGE: Alpha={a_r}, Beta={b_r} (Bereits vorhanden)")
            continue
        ...
        print(f"\n" + "="*60)
        print(f"STARTE NEUEN ZOOM-RUN (19 gesamt): Alpha={a_r}, Beta={b_r}")
        print(f"Run-Name: {RUN_NAME}")
        print("="*60 + "\n")

        # Modell & Optimizer neu instanziieren
        ...
        meta = make_meta_dict(
            ...
            extra={
                "alpha": a_r, "beta": b_r,
                "loss": f"triple_loss_zoom(a={a_r}, b={b_r})"
            }
        )
...
print("\n--- Alle 19 neuen Zoom-Punkte wurden erfolgreich trainiert! ---")