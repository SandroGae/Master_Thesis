...
BATCH_SIZE = 8
EPOCHS = 100
ALPHA_LIST = [0.0, 0.02, 0.04, 0.06, 0.08, 0.12]
...
for alpha_val in ALPHA_LIST:
    BASE_NAME = "unet_25d_MAE_MSE_RUN3"
    ...
    print(f"\n>>> Starte Training: {RUN_NAME} (Alpha={alpha_val})")
    ...
    model.compile(optimizer=optimizer, loss=get_mae_mse_loss(alpha_val), 
    ...
    meta = make_meta_dict(script_name=RUN_NAME, batch_size=BATCH_SIZE, epochs=EPOCHS, 
                          ...
                          extra={"loss": f"mae_mse(alpha={alpha_val})"})

    finalize_run(model, history, RUN_NAME, meta)

print("\nAlle Alpha-Tests abgeschlossen.")