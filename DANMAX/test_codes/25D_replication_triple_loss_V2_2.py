...
# NEU: Ein kleiner Puffer (288x288), damit wir stufenlos verschieben können!
...
def load_and_crop_4_quadrants_chunked(base_path, scan_id, start_img, end_img):
    ...
    with h5py.File(dark_file, 'r') as f_d, h5py.File(white_file, 'r') as f_w, h5py.File(ct_file, 'r') as f_ct:
        ...
        # Puffergröße laden
        ch, cw = LOAD_CROP_SIZE[0], LOAD_CROP_SIZE[1]
...
def prepare_cache():
    ...
    print(f"Erstelle 70/20/10 Cache (Chunked & Leakage-free, LOAD_SIZE {LOAD_CROP_SIZE[0]}x{LOAD_CROP_SIZE[1]})...")
    ...
    for mode, (s, e) in splits.items():
        ...
        np.save(paths[f"{prefix}_l"], data_x.reshape((-1, SERIES_LEN, LOAD_CROP_SIZE[0], LOAD_CROP_SIZE[1])))
        del data_x; gc.collect()
        
        data_y = load_and_crop_4_quadrants_chunked(BAMBOO_RAW, 32, s, e)
        np.save(paths[f"{prefix}_h"], data_y.reshape((-1, SERIES_LEN, LOAD_CROP_SIZE[0], LOAD_CROP_SIZE[1])))
        del data_y; gc.collect()

    return paths

# =====================================================
# 5. DATASET & GENERATOR (High Performance Batched)
# =====================================================
def create_tf_dataset(low_path, high_path, is_training=True):
    ...
    def tf_load_window(idx):
        ...
        x.set_shape([DEPTH, LOAD_CROP_SIZE[0], LOAD_CROP_SIZE[1], 1])
        y.set_shape([DEPTH, LOAD_CROP_SIZE[0], LOAD_CROP_SIZE[1], 1])
        return x, y

    def prepare_25d(x, y):
        return tf.transpose(tf.squeeze(x, -1), [1, 2, 0]), y[DEPTH//2]

    def augment_and_scale_batched(x, y):
        # x und y haben hier bereits die Form (BATCH_SIZE, 288, 288, Kanäle)
        if is_training:
            xy = tf.concat([x, y], axis=-1)

            # 1. Translation durch Random Crop (Blitzschnell)
            batch_dim = tf.shape(xy)[0]
            xy = tf.image.random_crop(xy, size=[batch_dim, CROP_SIZE[0], CROP_SIZE[1], DEPTH + 1])

            # 2. Native Flips & 90-Grad Rotation (Blitzschnell)
            ...
            # 3. Intensitätsskalierung (0.6 bis 1.0) pro Bild im Batch
            scale = tf.random.uniform([batch_dim, 1, 1, 1], 0.6, 1.0)
            return x * scale, y * scale
        else:
            xy = tf.concat([x, y], axis=-1)
            # Statischer Center Crop für Validation/Test
            xy = tf.image.resize_with_crop_or_pad(xy, CROP_SIZE[0], CROP_SIZE[1])
            x = xy[..., :DEPTH]
            y = xy[..., DEPTH:]
            return x, y
    ...
    # WICHTIG: Erst Batchen, dann Augmentieren! Das macht es super schnell.
    ds = ds.batch(BATCH_SIZE)
    ds = ds.map(augment_and_scale_batched, num_parallel_calls=tf.data.AUTOTUNE)
