...
def unet_2d_stacked(input_shape):
    ...
    out = layers.Conv2D(1, (1, 1), activation="linear", name="output")(c8)
...
def get_triple_loss(alpha, beta):
    def loss(yt, yp):
        # Sicherstellen, dass die Datentypen passen
...
# 2. Instanziere den Loss für P14
...
def load_and_crop_4_quadrants_chunked(base_path, scan_id, start_img, end_img, crop_dim):
    ...
    with h5py.File(dark_file, 'r') as f_d, h5py.File(white_file, 'r') as f_w, h5py.File(ct_file, 'r') as f_ct:
        ...
        for s in range(start_img, end_img, 100):
            e = min(s + 100, end_img)
            corrected = np.clip((ct_data[s:e].astype(np.float32) - m_dark) / denom, -0.1, 1.2)
...
def prepare_cache():
    ...
    for mode, (s, e) in splits.items():
        ...
        # NEU: Train bekommt 288x288 Puffer, Val/Test exakt 256x256
        c_size = LOAD_CROP_SIZE if mode == "train" else CROP_SIZE
        
        data_x = load_and_crop_4_quadrants_chunked(BAMBOO_RAW, 57, s, e, c_size)
        np.save(paths[f"{prefix}_l"], data_x.reshape((-1, SERIES_LEN, c_size[0], c_size[1])))
        del data_x; gc.collect()
        
        data_y = load_and_crop_4_quadrants_chunked(BAMBOO_RAW, 32, s, e, c_size)
...
def create_tf_dataset(low_path, high_path, is_training=True):
    ...
    def augment_and_scale(x, y):
        if is_training:
            ...
            # 2. Native Flips & Rotation (jetzt wieder pro Bild individuell!)
            ...
            # Val/Test sind durch das Caching bereits exakt 256x256, keine Anpassung nötig
            return x, y
    ...
    # WICHTIG: Augmentieren VOR dem Batching für maximale Diversität!
