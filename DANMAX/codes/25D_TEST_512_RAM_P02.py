...
import psutil  # NEU: Für die System-RAM Auslesung
...
# =====================================================
# 1. SETUP (Hart codiert auf P02 / Kein Job-Array)
# =====================================================
MY_POINT = "P02"
MY_ALPHA = 0.0
MY_BETA = 2.0/6.0
...
# OUTPUT ORDNER (Ich habe "_512" angehängt, damit die 256er Ergebnisse nicht überschrieben werden)
TEST_OUT_ROOT = SCRATCH_ROOT / "models" / "TEST_V2_triple_512"
...
SERIES_LEN = 40
BASEFILTERS = 64
CROP_SIZE = (512, 512) # NEU: Geändert von 256 auf 512
EPOCHS = 200
LR_TARGET = 2e-4
WARMUP_EPOCHS = 10
...
# NEU: Callback für Live RAM/VRAM Anzeige
class MemoryMonitorCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        try:
            ...
            vram_gb = vram_info['current'] / (1024**3)
            vram_str = f"{vram_gb:.2f} GB"
        except Exception:
            vram_str = "N/A"
        print(f" -> 🖥️ System-RAM: {ram_gb:.2f} GB | 🎮 GPU-VRAM: {vram_str}")

# =====================================================
# 5. MAIN LOOP
# =====================================================
def main():
    RUN_NAME = f"{MY_POINT}__a{MY_ALPHA:.4f}_b{MY_BETA:.4f}_seed{MY_SEED}_512px"
    print(f"\n{'='*60}")
    print(f"🚀 STARTE JOB | Punkt: {MY_POINT} | Seed: {MY_SEED} | Res: {CROP_SIZE}")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        callbacks = [
            ...
            MemoryMonitorCallback(), # NEU: Integriert für die Konsolenausgabe
