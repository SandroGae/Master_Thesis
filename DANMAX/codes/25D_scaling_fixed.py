...
# HIER IST DIE NEUE FUNKTION (Nur Min-Max Scaling)
def normalize_v2_minmax(max_val: float, p: float = 0.5):
    def map_volume(x, y):
        ...
        # Reine Division durch Max -> Werte liegen in [0, 1]
        x = x / max_val
        y = y / max_val

        # Sicherheitshalber clippen
...
# HIER DIE NEUE MAP-FUNKTION AUFRUFEN
train_ds = (tf.data.Dataset.from_tensor_slices((X_train, y_train))
            .shuffle(len(X_train), seed=SEED, reshuffle_each_iteration=True)
            .map(normalize_v2_minmax(max_val=MAX_PIXEL_VAL, p=0.5), num_parallel_calls=tf.data.AUTOTUNE)
            .map(prepare_25d_input, num_parallel_calls=tf.data.AUTOTUNE)
            .batch(BATCH_SIZE)
            .prefetch(tf.data.AUTOTUNE))

val_ds = (tf.data.Dataset.from_tensor_slices((X_val, y_val))
          .map(normalize_v2_minmax(max_val=MAX_PIXEL_VAL, p=0.0), num_parallel_calls=tf.data.AUTOTUNE)
          .map(prepare_25d_input, num_parallel_calls=tf.data.AUTOTUNE)
          .cache()
          .batch(BATCH_SIZE)
          .prefetch(tf.data.AUTOTUNE))
...
meta = make_meta_dict(
    ...
    extra={"normalization": "minmax(65535)", "loss": "mae_ssim(alpha=0.6)"}
)
