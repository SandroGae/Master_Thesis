...
# =====================================================
# DYNAMISCHE SKALIERUNG, AUGMENTATION & METRIKEN
# =====================================================
def get_dynamic_max(data_array):
    """Ermittelt automatisch die wahrscheinliche Bit-Tiefe anhand des Maximalwerts."""
    m = np.max(data_array)
    if m <= 255: return 255.0         
    elif m <= 4095: return 4095.0     
    elif m <= 65535: return 65535.0   
    else: return float(m)             

# UPDATE: Nimmt jetzt getrennte Max-Werte für X und Y
def augment_and_normalize_v3_proper(max_x: float, max_y: float, p: float = 0.5, aug_strength: float = 0.2):
    def map_volume(x, y):
...
train_ds = (tf.data.Dataset.from_tensor_slices((X_train, y_train))
            ...
            .map(augment_and_normalize_v3_proper(max_x=MAX_X, max_y=MAX_Y, p=0.5, aug_strength=0.2), num_parallel_calls=tf.data.AUTOTUNE)
            .map(prepare_25d_input, num_parallel_calls=tf.data.AUTOTUNE)
            .batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE))

val_ds = (tf.data.Dataset.from_tensor_slices((X_val, y_val))
          .map(augment_and_normalize_v3_proper(max_x=MAX_X, max_y=MAX_Y, p=0.0, aug_strength=0.0), num_parallel_calls=tf.data.AUTOTUNE)
          .map(prepare_25d_input, num_parallel_calls=tf.data.AUTOTUNE)
...
meta = make_meta_dict(
    ...
    extra={
        ...
        "normalization": f"minmax(X={MAX_X}, Y={MAX_Y})", "augmentation": "gamma+scale", 
