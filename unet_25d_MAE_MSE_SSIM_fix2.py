...
# --- REPRODUZIERBARKEIT ---
SEED_START = 44
MAX_SEED = 46  # Versuche mit Seed 44, 45 und 46 möglich
...
# --- GEZIELTE PUNKTE (Alpha, Beta) ---
# 4/12, 10/12 | 4/12, 1.0 | 10/12, 8/12
TARGET_PAIRS = [
    (4/12, 10/12),
    (4/12, 1.0),
    (10/12, 8/12)
...
for a_raw, b_raw in TARGET_PAIRS:
    # Sauberes Runden für die Pfadnamen
    a_r = round(float(a_raw), 4)
    b_r = round(float(b_raw), 4)

    current_seed = SEED_START
    run_finished = False
    
    while not run_finished and current_seed <= MAX_SEED:
        ...
        print(f"\n>>> START: Alpha={a_r} ({a_raw}), Beta={b_r} ({b_raw}), Seed={current_seed}")
        ...
        failed = was_aborted[0] or last_psnr < 25.0 
        ...
        if failed:
            print(f"\n!!! RUN FAILED (Seed {current_seed}) - Versuche nächsten Seed...")
            current_seed += 1
            if current_seed > MAX_SEED:
                print(f"!!! ALLE SEEDS FAILED für Alpha={a_raw}, Beta={b_raw}. Gehe zum nächsten Punkt.")
                run_finished = True
        else:
            print(f">>> SUCCESS für Alpha={a_raw}, Beta={b_raw} mit Seed {current_seed}")
            run_finished = True

        tf.keras.backend.clear_session(); import gc; gc.collect()

print("\n--- Gezielte Runs beendet ---")