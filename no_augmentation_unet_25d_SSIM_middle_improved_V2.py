# no_augmentation_unet_25d_SSIM_middle_improved_V2.py
#!/usr/bin/env python3
...
def normalize_and_scale_3d_per_slice(scale: float):
    ...
    Schritte je Sample:
      1) Clip >= 0
      2) Norm pro Slice: Division durch Summe über (H,W,C) -> Form (D,1,1,1)
      3) Fixe Skalierung je Sample: gleiche Skala alle Slices (Form (D,1,1,1))
    """
    def map_volume(x, y):
        x = tf.nn.relu(x)
        y = tf.nn.relu(y)
...
BASE_NAME = "no_augmentationunet_25d_SSIM_middle_improved_V2"
...
train_ds = (tf.data.Dataset.from_tensor_slices((X_train, y_train))
            ...
            .map(normalize_and_scale_3d_per_slice(10000.0), num_parallel_calls=tf.data.AUTOTUNE)
...
meta = make_meta_dict(
    ...
    scale_range_train=(10000,10000),
    scale_range_val=(10000,10000),
