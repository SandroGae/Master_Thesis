...
OUT_DIR  = ROOT_DIR / "Plots/Unet/Evaluation_Results"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Mapping: Rang_X -> (Alpha, Beta)
# Bitte ergänze hier deine restlichen Modelle (Rang 11 bis 41)
MODEL_MAP = {
    "Rang_1": (0.33, 0.17), "Rang_2": (0.17, 0.0), "Rang_3": (0.33, 0.33),
    "Rang_4": (0.17, 0.17), "Rang_5": (0.33, 0.0), "Rang_6": (0.17, 0.67),
    "Rang_7": (0.25, 0.08), "Rang_8": (0.25, 0.0), "Rang_9": (0.17, 0.5),
    "Rang_10": (0.25, 0.17)
}
...
def collect_all_metrics():
    all_results = []
    for rank, (alpha, beta) in MODEL_MAP.items():
        print(f"Calculating {rank}...")
        for s_id, cfg in SERIES_CONFIG.items():
            path = IN_DIR / f"Pred_{rank}_D5_S{s_id}_FullSeries.npz"
    ...
    # Aggregation pro Run und Richtung
...
def plot_heatmaps(df):
    ...
    for m_key, (m_title, cmap) in metrics.items():
        for d in ["H", "K", "L"]:
            ...
            plt.scatter(sub['alpha'], sub['beta'], c='white', edgecolors='black', s=40)
